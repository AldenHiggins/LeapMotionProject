...
public class SteamVR_Camera : MonoBehaviour
{
...
#else
	static readonly string[] headTypes = { "AudioListener", "GUILayer", "FlareLayer" };
#endif
	...
	{
		var camera = GetComponent<Camera>();
		var sceneTexture = SteamVR_Camera.GetSceneTexture(camera.hdr);
		if (SteamVR_Render.Top() == this)
		{
			var vr = SteamVR.instance;

			Graphics.SetRenderTarget(sceneTexture);
			SteamVR_Camera.blitMaterial.mainTexture = src;

			GL.PushMatrix();
			GL.LoadOrtho();
			SteamVR_Camera.blitMaterial.SetPass((int)QualitySettings.activeColorSpace);
			GL.Begin(GL.QUADS);
			...
			int i = (int)SteamVR_Render.eye;
			vr.compositor.Submit(SteamVR_Render.eye, src.GetNativeTexturePtr(), ref vr.textureBounds[i]);
		}
		else
		{
			Graphics.SetRenderTarget(sceneTexture);
			Graphics.Blit(src, sceneTexture);
		}
	}
